...
function copySlickCss() {
  return gulp
    .src("node_modules/slick-carousel/slick/{slick,slick-theme}.css")
    .pipe(gulp.dest("build"));
}

function copySlickFonts() {
  return gulp.src("node_modules/slick-carousel/slick/fonts/slick.*").pipe(gulp.dest("build/fonts"));
}

const copySlickAssets = gulp.parallel(copySlickCss, copySlickFonts);

function copyNoUiSliderAssets() {
  return gulp.src("node_modules/nouislider/distribute/nouislider.min.css").pipe(gulp.dest("build"));
}
